...
[![build](https://github.com/Young-BLUE/AiTravelPlanner/actions/workflows/build.yml/badge.svg)](https://github.com/Young-BLUE/AiTravelPlanner/actions/workflows/build.yml)
...
> **API 키 없이 바로 실행해 볼 수 있습니다.**
> 키가 없으면 **데모 모드**로 뜨고, 미리 만들어 둔 일정을 0.1초에 조회합니다.
> 새로 생성해야 하는 요청만 막히고 무엇이 가능한지 안내합니다. → [실행 방법](#실행-방법)
...
```
"도쿄 3박4일 여행"          ┐
"도쿄로 3박4일 다녀오려고 해"  ├→  도쿄|미지정|3|미지정|미지정|미지정|미지정
"도쿄 여행 3박 4일 추천해줘"   ┘
```
...
- **여행 시기는 계절로 뭉개 넣습니다.** 봄 교토와 겨울 교토는 같은 일정이면 안 됩니다.
  시기를 고르지 않은 요청은 `미지정` 한 칸에 모입니다 → [판단 기록](#여행-시기--오늘-날짜로-정하던-계절을-요청에서-받도록)
...
### API 키가 없을 때 — 조용한 실패에서 명시적 데모 모드로

SDK는 자격증명을 첫 요청 시점에 해석합니다. 그래서 키가 없어도 서버는 정상적으로 뜨고,
사용자가 버튼을 누를 때마다 401 → 502만 반복됩니다. 재시도로는 절대 해결되지 않는데
"다시 시도해 주세요"만 나오는, 가장 나쁜 실패 방식입니다.

처음에는 **기동을 중단**시켜 막았습니다. 하지만 이 서비스는 캐시에 미리 만들어 둔
일정을 이미 갖고 있습니다. **키가 없다고 아무것도 못 할 이유가 없습니다.**

그래서 지금은 **데모 모드**로 뜹니다.

| 요청 | 데모 모드 |
|---|---|
| 미리 생성된 일정 조회 | 정상 동작 (0.1초) |
| 자유 문장 입력 | 규칙 기반 추출로 대체 |
| 새 일정·추천 생성 | 차단 + 무엇이 가능한지 안내 |

조용한 실패와 다른 점은 **축소 모드임을 계속 알린다**는 것입니다. 기동 로그, 화면 상단
배너, 요청별 응답 메시지 세 곳에서 드러납니다.

자유 문장은 모델 대신 **규칙 기반 추출기**가 처리합니다. DB에 있는 도시명 164개와 대조하고
`(\d+)박` 같은 패턴으로 조건을 뽑습니다. Haiku만큼 유연하지는 않지만 캐시에 있는 일정을
찾아 주기에는 충분합니다.

```
규칙 기반 추출 - PLAN / 다낭 / 3박 / 800000원 / 가족 / 관심사[]
```
...
### 여행 시기 — 오늘 날짜로 정하던 계절을 요청에서 받도록

처음에는 캐시 키의 계절을 **요청이 들어온 날짜**로 정했습니다. 두 가지가 잘못돼 있었습니다.

- **시드가 계절이 바뀔 때마다 통째로 무효가 됩니다.** 9월에 만든 시드는 전부 `가을` 키라,
  12월 1일부터는 `겨울`로 조회되어 한 건도 맞지 않습니다. API 키 없는 데모 모드는 시드만으로
  동작하므로 인기 여행지 카드까지 전부 막힙니다. 날짜가 지나야 드러나는 문제라 CI로는 잡히지 않습니다
- **모델은 계절을 몰랐습니다.** 프롬프트 어디에도 시기가 들어가지 않아, 키만 계절별로 나뉘고
  내용은 같았습니다. 시기를 밝히지 않은 추천 요청에 모델이 임의로 "겨울 여행지"를 답한 시드도 있었습니다

그래서 계절을 **요청 내용**에서 받도록 바꿨습니다. 조건 선택에서는 월을 고르고,
자유 입력에서는 "1월", "벚꽃 보러" 같은 표현에서 추출합니다.

| 요청 | 캐시 키 | 프롬프트 |
|---|---|---|
| 시기 미선택 | `미지정` | 계절을 가정하지 않고 연중 방문 가능한 일정 |
| 4월 선택 | `봄` | `여행 시기: 4월` — 벚꽃·우기·계절 행사 반영 |

월은 모델에게 그대로 주고 키에는 계절로 뭉갭니다. 3월 초와 5월 말은 풍경이 다르지만,
월 단위로 키를 나누면 조합이 12배가 됩니다. 예산을 50만원 구간으로 묶은 것과 같은 판단입니다.
기존 시드는 계절 정보 없이 만들어진 것이라 재생성 없이 `미지정`으로 옮겼고,
키 문자열과 필드가 어긋나지 않는지 테스트로 확인합니다.
...
키 없이 띄우면 **데모 모드**로 동작합니다. 미리 만들어 둔 일정을 그대로 조회할 수 있습니다.
...
랜딩의 **인기 여행지 카드를 누르면** 바로 결과가 나옵니다. 조건 선택 탭에서 같은 조합을
고르거나, 자유 입력에 `"도쿄 3박4일 여행"`처럼 쳐도 됩니다.

키가 없으면 새로 생성하는 요청은 막히고, 무엇이 가능한지 안내가 나옵니다.
캐시에 무엇이 들어 있는지는 [`seed/itineraries.json`](backend/src/main/resources/seed/itineraries.json)에서
`cacheKey` 목록으로 확인할 수 있습니다.
...
캐시 HIT [도쿄|미지정|3|미지정|미지정|미지정|미지정] - 누적 3회
```
...
- **시드에는 계절이 캐시 키로 들어갑니다.** 봄 교토와 겨울 교토를 같은 일정으로
  주지 않기 위한 설계지만, 계절이 바뀌면 기존 시드가 전부 미스가 됩니다.
  분기마다 `export` 프로파일로 다시 내보내야 합니다
